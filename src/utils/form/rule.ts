...
import { REGEXP_CODE_SIX, REGEXP_EMAIL, REGEXP_PHONE, REGEXP_PWD2, REGEXP_NAME, REGEXP_ACCOUNT } from '@/config';
...
interface CustomFormRules {
  ...
  email: FormItemRule[];
	/** 校验登录密码 */
	checkPwd: FormItemRule[];
	/** 校验登录账号 */
	checkUserName:  FormItemRule[];
}
...
export const formRules: CustomFormRules = {
    ...
    { pattern: REGEXP_NAME, message: '用户名格式错误', trigger: 'input' }
	],
	account: [
		createRequiredFormRule('请输入账号'),
    { pattern: REGEXP_ACCOUNT, message: '账号为6-8位字母和数字组合', trigger: 'input' }
	],
  phone: [
    createRequiredFormRule('请输入手机号码'),
    { pattern: REGEXP_PHONE, message: '请输入11位有效的手机号码', trigger: 'input' }
  ],
  pwd: [
    createRequiredFormRule('请输入密码'),
		{ pattern: REGEXP_PWD2, message: '密码为6-12位，包含大小写字母、数字和特殊字符组合', trigger: 'input' }
